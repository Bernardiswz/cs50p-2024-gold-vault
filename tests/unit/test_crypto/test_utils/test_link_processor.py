...
class TestLinkProcessor:
    ...
    def test_get_link_path_no_link(self, link_processor: LinkProcessor) -> None:
        with patch("os.path.islink", return_value=False), patch("os.path.realpath", return_value="/path/to/real/file"):
    ...
    def test_get_link_path_simple_link(self, link_processor: LinkProcessor) -> None:
        with patch("os.path.islink", side_effect=[True, False]), patch(
            "os.readlink", return_value="/path/to/real/file"
    ...
    def test_get_link_path_cyclic_link(self, link_processor: LinkProcessor) -> None:
        with patch("os.path.islink", side_effect=[True, True, True]), patch(
            "os.readlink", side_effect=["/path/to/link1", "/path/to/link2", "/path/to/link1"]
        ):
    ...
    def test_get_link_path_max_depth_exceeded(self, link_processor: LinkProcessor) -> None:
        with patch("os.path.islink", return_value=True), patch("os.readlink", return_value="/path/to/link"):
